...
namespace SDP_T01_Group06
{
    class Program
    {
        ...
        static void LoggedInMenu(User currentUser, List<Document> allDocuments)
        {
            ...
            CreateDocumentCommand createDocument = new CreateDocumentCommand(currentUser, allDocuments);
            //SubmitForApprovalCommand submitCommand = new SubmitForApprovalCommand(currentUser, allDocuments);
            ...
            while (true)
            {
                ...
                {
                    case "Create a new document":
                        invoker.setCommand(createDocument);
                        //CreateNewDocument(currentUser, allDocuments);
                        break;
                    case "Edit existing document":
                        EditExistingDocument(currentUser, allDocuments);
